...
1. URLパラメータ指定(PNG形式専用)  
   ...
   この方式で画像読み込みが成功した場合、以下の下書き使用時のルールが適用されます。
...
2. 起動オプション指定  
   ...
   ```   
   この方式での画像読み込みは、新規キャンバスと同等の扱いとなり、下書き使用時のルールは適用されません。  
   PNG形式以外のファイルも読み込み可能ですが、ツールが出力する画像はすべてPNG形式になります。

共通事項
* キャンバスサイズは画像ファイルのサイズに合わせて変更されます。サイズが規定値を超える場合、超過分は切り捨てられます。
