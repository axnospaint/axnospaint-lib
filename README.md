...
* 投稿情報の`strOekakiId`に、URLパラメータで指定した`oekaki_id`の値が設定される。（下書きを利用した投稿であることを示す）
...
|  strOekakiId  | 「下書き機能」を使用した場合に付与される識別子。未使用時はNULL。（URLパラメータで指定した`oekaki_id`の内容の転記。**ただし、AXNOSPaint使用中の操作で更新される場合があります。** |
