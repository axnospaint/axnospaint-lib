...
export default class Mascot {
  ...
  init() {
    // HTMLデータを展開
    // 設定画面ナビボタン
    document.getElementById('axp_config_button_version').insertAdjacentHTML(
      'beforebegin',
      `
    ...
    // 設定画面本文
    document.getElementById('axp_config_div_version').insertAdjacentHTML(
      'beforebegin',
      `
    ...
    // 本体
    document.getElementById('axp_canvas').insertAdjacentHTML(
      'beforeend',
      `
            <div id="axp_mascot">
                <div>
  ...
  startEvent() {
    ...
    const messages = document.querySelectorAll(
      '.axpc_MSG,.axpc_FUNC,#axp_palette_div_paletteBox',
    );
    ...
    document
      .getElementById('axp_layer_ul_layerBox')
      .addEventListener('pointermove', (e) => {
        let element = document.elementFromPoint(e.clientX, e.clientY);
        let targetClass = null;
        if (
          element.classList.contains('axpc_icon_eyeON') ||
          element.classList.contains('axpc_icon_eyeOFF')
        ) {
          targetClass = 'axpc_icon_eyeON';
        }
        if (
          element.classList.contains('axpc_icon_lockON') ||
          element.classList.contains('axpc_icon_lockOFF')
        ) {
          targetClass = 'axpc_icon_lockON';
        }
        if (
          element.classList.contains('axpc_icon_maskON') ||
          element.classList.contains('axpc_icon_maskOFF')
        ) {
        ...
        if (
          element.classList.contains('axpc_layer_div_cellRightSide') ||
          element.classList.contains('axpc_layer_span_cellName')
        ) {
    ...
    document
      .getElementById('axp_launcher_div_buttons')
      .addEventListener('pointermove', (e) => {
    ...
    // 設定：アクペたん
    document.getElementById('axp_config_form_mascotMessageType').onchange = (
      e,
    ) => {
      this.setMsg();
    };
    ...
    // 設定：アクペたん
    document
      .getElementById('axp_mascot_char')
      .addEventListener('pointerdown', (e) => {
        e.stopPropagation();
        const rect = this.element.getBoundingClientRect();
        const rectHead = document
  ...
  displayMascot() {
    ...
    if (type === 'none') {
      document.getElementById(
        'axp_config_form_mascotMessageType',
      ).style.opacity = '.3';
      this.hide();
    } else {
      document.getElementById(
  ...
  randomMessage(type = 'ETC') {
    ...
    while (index === -1) {
      ...
      if (length > 1 && this.lastType === execType && this.lastIndex === rand) {
