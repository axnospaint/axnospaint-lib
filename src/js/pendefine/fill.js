...
export class Fill extends PenObj {
  ...
  // 描画終了
    ...
    if (this.axpObj.isDrawing && !this.axpObj.isDrawCancel) {
      ...
      // 範囲外の場合は処理しない
      if (
        inRange(x, 0, this.axpObj.x_size) &&
        inRange(y, 0, this.axpObj.y_size)
      ) {
        ...
        var img_input;
        if (
          document.getElementById('axp_pen_select_fillMode').value ===
          'option_layer'
        ) {
          // 判定対象：現在レイヤー
        ...
        this.regionFill(img_input, img_output, x, y, rgbCode);
...
Fill.prototype.regionFill = function (img_input, img_output, x, y, fillColor) {
  let oversize = Number(
    document.getElementById('axp_pen_range_fillThreshold').value,
  );
  ...
  const compareColor = (
    ...
    if (isAlpha) {
      ...
      if (currentAlpha === 0) {
        ...
        // 現在：有色
        if (
          currentColorRGB[0] === selectColorRGB[0] &&
  ...
  while (pxlArr.length) {
    ...
    if (Processed[p.y * img_input.width + p.x] === true) {
      ...
      if (
        compareColor(img_input, p.x, p.y, selectColorRGB, isAlpha, Processed)
      ) {
        // 座標の画素を処理済みにする
