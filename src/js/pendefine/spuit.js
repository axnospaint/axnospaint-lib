...
export class Spuit extends PenObj {
  ...
  move(x, y, e) {
    ...
    if (
      inRange(x, -2, this.axpObj.x_size - 1 + 2) &&
      inRange(y, -2, this.axpObj.y_size - 1 + 2)
    ) {
      // sx0,sy0:取り出す矩形の左上座標
      ...
      const imagedata = this.axpObj.CANVAS.main_ctx.getImageData(
        sx0,
        sy0,
        sw,
        sh,
    ...
    if (e.target.id === this.axpObj.CANVAS.main.id) {
      // 座標のドットを読み取る
      const imagedataSpuitPonit = this.axpObj.CANVAS.main_ctx.getImageData(
        x,
        y,
        1,
        1,
