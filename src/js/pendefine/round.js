...
export class Round extends PenObj {
  ...
  init_brush(option) {
    ...
    if (
      ...
      this.CANVAS.brush_ctx.strokeStyle = createTonePattern(
        this.toneLevel,
        this.getColor(),
      );
    } else {
    ...
    switch (this.drawmode) {
      case this.axpObj.CONST.DRAW_FREEHAND: {
        ...
        const stabilizer_value = Number(
          document.getElementById('axp_config_form_stabilizerValue').volume
            .value,
        );
        ...
        if (this.axpObj.isLine) {
          this.CANVAS.brush_ctx.beginPath();
          this.CANVAS.brush_ctx.moveTo(firstPoint.x, firstPoint.y);
        }
        if (isStabilizer) {
          // 手ぶれ補正
          ...
          // 補正なし
          this.CANVAS.brush_ctx.lineTo(currentPoint.x, currentPoint.y);
        }
        break;
      }
