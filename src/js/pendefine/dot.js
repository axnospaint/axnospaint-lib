...
export class Dot extends Round {
  ...
  getCursorPositionDot(e) {
    ...
    let t_canvas_x = Math.floor(
      ((e.clientX - crect.left) * 100) / this.axpObj.scale,
    );
    let t_canvas_y = Math.floor(
      ((e.clientY - crect.top) * 100) / this.axpObj.scale,
    );
    let t_width = this.size;
    let t_dot_x = Math.floor(t_canvas_x / t_width) * t_width;
    let t_dot_y = Math.floor(t_canvas_y / t_width) * t_width;
    let x =
      parseInt((t_dot_x * this.axpObj.scale) / 100) + crect.left - rect.left;
    let y =
      parseInt((t_dot_y * this.axpObj.scale) / 100) + crect.top - rect.top;
  ...
  draw() {
    ...
    // 終了点（今回入力座標）
    end_x = this.trans_dot(
      this.input_position[this.input_position.length - 1].x,
    );
    end_y = this.trans_dot(
      this.input_position[this.input_position.length - 1].y,
    );
    switch (this.drawmode) {
      case this.axpObj.CONST.DRAW_FREEHAND:
        ...
        if (this.axpObj.isLine) {
          ...
          start_x = this.trans_dot(
            this.input_position[this.input_position.length - 2].x,
          );
          start_y = this.trans_dot(
            this.input_position[this.input_position.length - 2].y,
          );
        }
        // 直線
        this.drawLine(
  ...
  drawLine(imageData, x0, y0, x1, y1, lineWidth, color) {
  ...
  // 円描画
  drawCircle(imageData, x0, y0, x1, y1, lineWidth, color) {
    const { width, data } = imageData;
