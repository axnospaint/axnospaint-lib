...
export class PostSystem {
  ...
  init() {
    ...
    this.CANVAS.thumbnail = document.getElementById(
      'axp_post_canvas_thumbnail',
    );
  ...
  resetCanvas() {
    ...
    // キャンバスサイズの表示
    document.getElementById('axp_post_span_imageSize').textContent =
      `${this.axpObj._('@COMMON.WIDTH')}:${x} ${this.axpObj._('@COMMON.HEIGHT')}:${y}`;
  }
  startEvent() {
    // 投稿フォームのカスタマイズ
    // 投稿フォーム
    if (!this.axpObj.postForm.input.isDisplay) {
      ...
      if (!this.axpObj.postForm.input.strName.isDisplay) {
        ...
        // 最大文字数設定
        document.getElementById('axp_post_text_name').maxLength =
          this.axpObj.postForm.input.strName.maxLength;
        // プレースホルダー設定
        document.getElementById('axp_post_text_name').placeholder =
      ...
      if (!this.axpObj.postForm.input.strTitle.isDisplay) {
        ...
        // 最大文字数設定
        document.getElementById('axp_post_text_title').maxLength =
          this.axpObj.postForm.input.strTitle.maxLength;
        // プレースホルダー設定
        document.getElementById('axp_post_text_title').placeholder =
      ...
      if (!this.axpObj.postForm.input.strMessage.isDisplay) {
        ...
        // 最大文字数設定
        document.getElementById('axp_post_textarea_message').maxLength =
          this.axpObj.postForm.input.strMessage.maxLength;
        // プレースホルダー設定
        document.getElementById('axp_post_textarea_message').placeholder =
    ...
    document.getElementById('axp_post_button_upload').onclick = () => {
      ...
      if (this.axpObj.postForm.input.strName.isInputRequired) {
        if (
          document.getElementById('axp_post_text_name').value.trim().length < 1
        ) {
          document.getElementById('axp_post_span_message').textContent =
            `${this.axpObj._('@POST.INFO_REQUIRED')} ( ${this.axpObj._('@POST.NAME')} )`;
          return;
        }
      }
      // タイトル
      if (this.axpObj.postForm.input.strTitle.isInputRequired) {
        if (
          document.getElementById('axp_post_text_title').value.trim().length < 1
        ) {
          document.getElementById('axp_post_span_message').textContent =
            `${this.axpObj._('@POST.INFO_REQUIRED')} ( ${this.axpObj._('@POST.TITLE')} )`;
          return;
        }
      }
      // 本文
      if (this.axpObj.postForm.input.strMessage.isInputRequired) {
        if (
          document.getElementById('axp_post_textarea_message').value.trim()
            .length < 1
        ) {
          document.getElementById('axp_post_span_message').textContent =
            `${this.axpObj._('@POST.INFO_REQUIRED')} ( ${this.axpObj._('@POST.MESSAGE')} )`;
          return;
        }
      }
      ...
      let objPostData = {
        ...
        strMessage: document
          .getElementById('axp_post_textarea_message')
          .value.trim(),
        strWatchList: document.getElementById('axp_post_checkbox_watchList')
          .checked
          ? 't'
          : '',
    ...
    // ボタンラベル初期設定
    document.getElementById('axp_post_button_upload_label').textContent =
      this.axpObj._('@POST.BUTTON_SUBMIT');
  }
}
