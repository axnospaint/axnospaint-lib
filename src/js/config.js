...
export class ConfigSystem {
  ...
  optionlist = [
    ...
    {
      value: 'func_switch_axp_penmode_eraser_round',
      name: '消しゴム種別:消しゴム',
    },
    {
      value: 'func_switch_axp_penmode_eraser_dot',
      name: '消しゴム種別:角消しゴム',
    },
    { value: '/optgroup' },
    ...
    {
      value: 'func_switch_axp_penmode_fillgradation',
      name: 'バケツ種別:階調バケツ',
    },
    { value: '/optgroup' },
  ...
  init() {
    ...
    targetElement.insertAdjacentHTML(
      'afterbegin',
      this.axpObj.translateHTML(htmldata),
    );
    // バージョン情報の表示
    document.getElementById('axp_config_div_versionInfo').textContent =
      `${this.axpObj.CONST.APP_TITLE} version ${PACKAGE_VERSION} (${PACKAGE_DATE})`;
  }
  // HTML展開
  deployHTML() {
    // カテゴリヘッダ生成
    ...
    // カスタムボタン用セレクトボックスの生成
    const elementsTbodyCustom = document.querySelector(
      '#axp_config_table_customButton>tbody',
    );
    ...
    const elementsTbody = document.querySelector(
      '#axp_config_table_shortcutKey>tbody',
    );
  ...
  startEvent() {
    ...
    const activateButton = (element) => {
      ...
      const currentActiveIndex = document.querySelector(
        '#axp_config_div_navButton .axpc_ACTIVE',
      );
      ...
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          const elementButton = document.querySelector(
            `#axp_config_div_navButton > button:nth-of-type(${entry.target.dataset.idx})`,
          );
    ...
    /*
        ...
        */
    // ボタンによるページジャンプ
    const elementsNavButton = document.querySelectorAll(
      '#axp_config_div_navButton button',
    );
      ...
      item.addEventListener('change', (e) => {
        ...
        switch (e.target.type) {
          ...
          case 'number': {
            ...
            value = adjustInRange(Number(value), e.target.min, e.target.max);
            // 要素の値を補正後の値に更新
    ...
    const inputHeight = document.getElementById(
      'axp_config_number_oekakiHeight',
    );
    ...
    document.getElementById('axp_config_button_newCanvas').onclick = () => {
      // キャンバスサイズの範囲チェック
      let x = (inputWidth.value = this.axpObj.checkCanvasSize_x(
        inputWidth.value,
      ));
      let y = (inputHeight.value = this.axpObj.checkCanvasSize_y(
        inputHeight.value,
      ));

      // 確認ダイアログ表示
      confirmExPromise(
        `現在の描画内容を破棄して新規キャンバス（${x}×${y}）を作成します。\nよろしいですか？\n（※この処理はアンドゥできません）`,
      )
        .then(() => {
          // ※OK時の処理
          // タブをキャンバスに変更
    ...
    document.getElementById(
      'axp_config_button_changeCanvasSize',
    ).onpointerdown = () => {
    ...
    // ウィンドウ位置リセット
    document
      .getElementById('axp_config_button_resetWindow')
    ...
    // アイコンサイズ
    document.getElementById('axp_config_form_minimizeButtonType').onchange = (
      e,
    ) => {
      this.axpObj.launcher.setButtonSize(e.target.value);
    };
    // 単一ウィンドウモード
    document.getElementById('axp_config_checkbox_singleWindowMode').onchange = (
      e,
    ) => {
    ...
    // レンジスライダー：手ぶれ補正 連動
    // 設定タブ側
    document.getElementById('axp_config_form_stabilizerValue').onchange = (
      e,
    ) => {
      document.getElementById('axp_pen_form_stabilizer').volume.value =
        e.target.value;
      document.getElementById('axp_pen_form_stabilizer').result.value =
        e.target.value;
    };
    // ペンツール側
    document.getElementById('axp_pen_form_stabilizer').onchange = (e) => {
      document.getElementById('axp_config_form_stabilizerValue').volume.value =
        e.target.value;
      document.getElementById('axp_config_form_stabilizerValue').result.value =
        e.target.value;
    ...
    // 使用する色作成ツール
    document
      .getElementById('axp_config_form_makeColorTypeRGB')
      .addEventListener('change', () => {
        this.axpObj.colorMakerSystem.updateMakeColorType();
      });
    document
      .getElementById('axp_config_form_makeColorTypePicker')
      .addEventListener('change', () => {
        this.axpObj.colorMakerSystem.updateMakeColorType();
      });
    document
      .getElementById('axp_config_form_makeColorTypeMixed')
      .addEventListener('change', () => {
        this.axpObj.colorMakerSystem.updateMakeColorType();
      });
    // ◆カラーパレット ----------------------------------------------------------------
    ...
    // カラーパレットの列数
    document
      .getElementById('axp_config_form_paletteColumnValue')
    ...
    document.getElementById('axp_config_button_saveColor').onclick = () => {
      ...
      for (const color of this.axpObj.colorPaletteSystem.currentPalette
        .palette) {
        colortext.push(color + '\n');
      }
      var filename =
        'ap_color' + dispDate(new Date(), 'YYYYMMDD_hhmmss') + '.txt';
    ...
    document.getElementById('axp_config_file_loadColor').onchange = (e) => {
      ...
      reader.onload = () => {
          ...
          if (data.substr(0, 6) === 'column') {
            ...
            if (
              ...
              !inRange(
                Number(column),
                0,
                this.axpObj.colorPaletteSystem.CONST.COLOR_MAX,
              )
            ) {
              alert(
                'columnが不正な値のため、読込を中止しました。\n行数:' +
                  (i + 1) +
                  ' 内容:' +
                  data,
              );
        ...
        this.axpObj.colorPaletteSystem.currentPalette.column =
          newPalette.column;
        // カラーパレット生成
    ...
    document.getElementById('axp_config_button_resetColor').onclick = () => {
      ...
      confirmExPromise(
        ...
        .then(() => {
          ...
          this.dispPalettebox(
            document.getElementById('axp_config_div_paletteBox'),
            this.axpObj.colorPaletteSystem.currentPalette,
          );
          // DBへ保存
          this.axpObj.saveSystem.save_palette(
            this.axpObj.colorPaletteSystem.currentPalette.palette,
          );
    ...
    // 拡大率追加
    document
      .getElementById('axp_config_button_addScale')
      .addEventListener('click', () => {
        const inputValue = Number(
          document.getElementById('axp_config_number_scale').value,
        );
        ...
        if (inRange(inputValue, min, max)) {
          // 挿入位置をサーチ
          const elements_li = document.querySelectorAll(
    ...
    // 拡大率削除
    document
      .getElementById('axp_config_button_deleteScale')
      .addEventListener('click', () => {
        const elements_li = document.querySelectorAll(
          '#axp_config_ul_scale>li',
        );
    ...
    // 拡大率初期化
    document
      .getElementById('axp_config_button_resetScale')
      .addEventListener('click', () => {
        // 確認ダイアログ表示
        confirmExPromise('拡大率の設定を初期状態に戻します。\nよろしいですか？')
          .then(() => {
            ...
            // キーカスタマイズセレクトボックス連動
            this.updateKeyCustomizationScaleTable(
              this.axpObj.currentScaleTable,
            );
            // コンフィグ保存
            this.saveConfig('SCALE', this.axpObj.currentScaleTable);
    ...
    // 割り当て無しのキーを非表示にする
    document
      .getElementById('axp_config_checkbox_shortcutKeyHiddenNofunc')
      .addEventListener('change', () => {
        this.switchNofuncKeytable();
      });
    // ショートカットのファイル保存
    document
      .getElementById('axp_config_button_saveShortcut')
      .addEventListener('click', () => {
        // セーブテキスト
        ...
        for (const tr of elementsTr) {
          let text =
            tr.id.slice('axp_config_custom_key'.length) +
            ':' +
            this.getSaveCustomText(tr.id);
          saveText.push(text + '\n');
        }
        const filename =
          'ap_shortcut' + dispDate(new Date(), 'YYYYMMDD_hhmmss') + '.txt';
      ...
      reader.onload = () => {
        ...
        for (let i = 2; i < loopend; i++) {
          ...
          if (document.getElementById(elememtId)) {
            // 親セレクトボックス要素を取得
            const elemSelectParent = document.querySelector(
              `#${elememtId} select:nth-of-type(1)`,
            );
            // valueの値を分解
            ...
            alert(
              'ショートカットファイルの内容に誤りがあるため、読込を中止しました。\n行数:' +
                (i + 1) +
                ' 内容:' +
                arr[i],
            );
      ...
      confirmExPromise(
        ...
        .then(() => {
          ...
          for (let i = 0; i < objKeyFunction.length; i++) {
            ...
            const selectMain = document.querySelector(
              `#${id} select:nth-of-type(1)`,
            );
    ...
    // デバッグ情報表示チェックボックス
    document.getElementById('axp_config_checkbox_useDebugMode').onchange = (
      e,
    ) => {
  ...
  getSaveCustomText(id) {
    ...
    switch (selectMain.value) {
      case 'func_loupe':
        // 拡大率
        inputScaleValue.value = adjustInRange(
      ...
      case 'func_size':
        // ペンの太さ
        // 数値範囲丸め
        inputSizeValue.value = adjustInRange(
  ...
  selectCustom(id, setValue = null) {
    ...
    if (elem.dataset.linkIndex) {
      // index番目のボタン要素を取得
      const elementButton = document.querySelector(
        `#axp_custom_div_buttons>button:nth-of-type(${elem.dataset.linkIndex})`,
      );
  ...
  switchNofuncKeytable() {
    const checked = document.getElementById(
      'axp_config_checkbox_shortcutKeyHiddenNofunc',
    ).checked;
    const elementTable = document.getElementById(
      'axp_config_table_shortcutKey',
    );
  ...
  createKeyConfigHTML(id, keybind) {
    ...
    inputSizeValue.setAttribute(
      'id',
      `axpc_config_number_sizeValue_${id}${idx}`,
    );
    ...
    inputScaleValue.setAttribute(
      'id',
      `axpc_config_number_scaleValue_${id}${idx}`,
    );
  ...
  addCanvasSizeHistory(x, y) {
    ...
    // 最大件数を超える場合、末尾１件削除
    if (
      this.configCanvasSizeHistory.length > this.CONST.CANVAS_SIZE_HISTORY_MAX
    ) {
  ...
  createHTMLListCanvasSizeHistory(value) {
    ...
    list.onclick = (e) => {
      ...
      document.getElementById('axp_config_number_oekakiWidth').value =
        Number(x);
      document.getElementById('axp_config_number_oekakiHeight').value =
        Number(y);
    };
    return list;
  }
  ...
  set_longtap_use() {
    if (this.axpObj.config('axp_config_form_useLongtap') === 'on') {
      // 補正の強さレンジスライダー有効
      document.getElementById(
        'axp_config_form_longtapDurationValue',
      ).volume.disabled = false;
      document.getElementById(
        'axp_config_form_longtapDurationValue',
      ).style.opacity = '1';
      document.getElementById(
        'axp_config_form_longtapStabilizerValue',
      ).volume.disabled = false;
      document.getElementById(
        'axp_config_form_longtapStabilizerValue',
      ).style.opacity = '1';
  ...
  restoreConfig(map) {
    ...
    const setRadioValue = (id, value) => {
      ...
      let isFound = false;
      const elementsRadio = document.querySelectorAll(
        `#${id} input[type="radio"]`,
      );
    ...
    map.forEach((value, key) => {
      ...
      switch (dataType) {
        ...
        case 'TOGSW':
          if (document.getElementById(elememtId)) {
            document.getElementById(elememtId).querySelector('input').checked =
              value;
          } else {
            isAvailable = false;
          }
        ...
        case 'GRIDC':
          if (document.getElementById(elememtId)) {
            ...
            document.getElementById(elememtId).style.backgroundColor =
              value.substring(0, 7);
          } else {
            isAvailable = false;
          }
        ...
        case 'WDPOS':
          // 初期化する設定の場合、復元を行わない
          if (
            this.axpObj.config('axp_config_form_saveLastWindowPosition') ===
            'off'
          ) {
            // スキップしたことを記憶
            isSkiped = true;
            ...
            // wdpos[1]:top
            if (
              !this.axpObj.dragWindow.setInitPosition(
                elememtId,
                wdpos[0],
                wdpos[1],
              )
        ...
        case 'P-RAD':
          // 初期化する設定の場合、復元を行わない
          if (
            this.axpObj.config('axp_config_form_saveLastPenValue') === 'off'
          ) {
            // スキップしたことを記憶
            isSkiped = true;
          } else {
  ...
  updateKeyCustomizationScaleTable(table) {
    const elementsSelectScaleTable = document.querySelectorAll(
