...
class UndoObj {
  constructor(obj) {
    ...
    // レイヤー統合時の統合先レイヤー
    this.layerObj_dest = obj.layerObj_dest
      ? new Layerdata(obj.layerObj_dest)
      : null;
  }
}
    ...
    switch (actionObj.type) {
        ...
        this.axpObj.undoSystem.setRedo({
          ...
          layerObj:
            this.axpObj.layerSystem.layerObj[
              this.axpObj.layerSystem.getLayerIndex(actionObj.layerObj.id)
            ],
        });
        // アンドゥ処理
        this.axpObj.layerSystem.setImageId(
          actionObj.layerObj.image,
          actionObj.layerObj.id,
        );
        break;
      ...
      case 'layer-integrate': {
        ...
        this.axpObj.undoSystem.setRedo({
          ...
          layerObj_dest:
            this.axpObj.layerSystem.layerObj[
              this.axpObj.layerSystem.getLayerIndex(actionObj.layerObj_dest.id)
            ],
        });
        // アンドゥ処理
        //  統合元レイヤーを復元
