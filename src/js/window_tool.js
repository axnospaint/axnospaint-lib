...
export class AssistToolSystem extends ToolWindow {
  ...
  startEvent() {
    ...
    const thumbnailHandMove = (e) => {
      ...
      const curX = adjustInRange(
        e.clientX - rectThumbnail.left - 6,
        4,
        this.CONST.X_LOUPE_MAX - 4 - 1,
      );
      const curY = adjustInRange(
        e.clientY - rectThumbnail.top - 6,
        4,
        this.CONST.Y_LOUPE_MAX - 4 - 1,
      );
      ...
      const dy =
        (curY * rangeHeight) / this.CONST.Y_LOUPE_MAX - rangeHeight / 2;
    ...
    // ドラッグ開始
    document.getElementById('axp_tool_div_minimap_outer').onpointerdown = (
      e,
    ) => {
    ...
    // ドラッグ中
    document.getElementById('axp_tool_div_minimap_outer').onpointermove = (
      e,
    ) => {
    ...
    const inputRangeGridVH = (e) => {
      ...
      this.axpObj.configSystem.saveConfig(
        'RANGE_axp_tool_form_gridH',
        elementFormGridH.volume.value,
      );
      this.axpObj.configSystem.saveConfig(
        'RANGE_axp_tool_form_gridV',
        elementFormGridV.volume.value,
      );
    };
    document
      .getElementById('axp_tool_range_gridH')
      .addEventListener('input', inputRangeGridVH);
    document
      .getElementById('axp_tool_range_gridV')
      .addEventListener('input', inputRangeGridVH);

    // 縦横連動チェックボックス
    document
      .getElementById('axp_tool_checkbox_gridVHLink')
      .addEventListener('change', (e) => {
        // チェックした時、横と縦の値を同値にする
        if (e.target.checked) {
          const newValue = document.getElementById(
            'axp_tool_range_gridH',
          ).value;
          document.getElementById('axp_tool_form_gridV').volume.value =
            newValue;
          document.getElementById('axp_tool_form_gridV').result.value =
            newValue;
          this.axpObj.updateGrid();
          this.axpObj.configSystem.saveConfig(
            'RANGE_axp_tool_form_gridV',
    ...
    document
      .getElementById('axp_tool_number_gridPixel1')
      .addEventListener('change', inputRangeGridPX);
    document
      .getElementById('axp_tool_number_gridPixel2')
      .addEventListener('change', inputRangeGridPX);

    // 共通部品トグル式スイッチ
    const elementTogglebuttons = document.querySelectorAll(
      '.axpc_toggle_switch',
    );
    ...
    // 補助線サブメニュー表示切替
    document
      .getElementById('axp_tool_button_gridConfig')
    ...
    // 色選択
    const elementInputColor = document.querySelectorAll(
      '.axp_common_inputcolor',
    );
    for (const elem of elementInputColor) {
      elem.addEventListener('click', (e) => {
        ...
        let alpha = Math.round(
          (100 * parseInt(colorcode.substring(7, 9), 16)) / 255,
        );
        const elementRange = document.getElementById(
          'axp_gridconfig_form_gridColor',
        );
    ...
    // 不透明度
    document
      .getElementById('axp_gridconfig_range_gridColor')
    ...
    document
      .querySelector('#axp_gridconfig>div')
      .addEventListener('click', (e) => {
        // サブウィンドウ内クリックでウィンドウが閉じないように、親へのイベント伝播を中止
        e.stopPropagation();
      });
    document
      .getElementById('axp_gridconfig_button_confirm')
  ...
  changeGridColor() {
    ...
    let alpha = Math.trunc(
      (255 * document.getElementById('axp_gridconfig_range_gridColor').value) /
        100,
    );
    ...
    // コンフィグ保存
    this.axpObj.configSystem.saveConfig(
      `GRIDC_${this.elementProcessingColor.id}`,
      colorcode,
    );
  }
  ...
  grid() {
    ...
    // ボタンの属性を更新することで、適用CSSを変更し、ボタンを凹ませる
    document.getElementById('axp_tool_button_grid').dataset.selected = this
      .isGrid
      ? 'true'
      : 'false';
    if (this.isGrid) {
  ...
  transparent() {
    ...
    document.getElementById('axp_tool_button_transparent').dataset.selected =
      this.isTransparent ? 'true' : 'false';
    ...
    // 投稿フォーム連動
    let input_transparent = document.querySelectorAll(
      'input[name=axp_post_radio_bg]',
    );
