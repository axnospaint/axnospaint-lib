...
export class Launcher extends ToolWindow {
  ...
  startEvent() {
    // 個別のウィンドウボタン
    const personalButtons = document.querySelectorAll(
    ...
    allButton.addEventListener('click', (e) => {
      if (e.target.classList.contains('axpc_launcher_minimize')) {
        ...
        e.target
          .querySelector('div')
          .classList.remove('axpc_icon_window_all_up');
    ...
    // ボタンサイズ
    this.setButtonSize(
      this.axpObj.config('axp_config_form_minimizeButtonType'),
    );
  }
  // ツールウィンドウのIDに対応するボタンを最小化状態にする
  minimizeButton(id) {
    const buttons = document.querySelectorAll(
      '.axpc_launcher_personalButton,.axpc_launcher_allButton',
    );
