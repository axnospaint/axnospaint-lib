...
export class SaveSystem {
  ...
  createSlotHTML(mode, cursor) {
    ...
    if (value.created !== undefined) {
      ...
      stringDate = stringDate.replace(
        /MM/,
        ('0' + (value.created.getMonth() + 1)).slice(-2),
      );
      stringDate = stringDate.replace(
        /DD/,
        ('0' + value.created.getDate()).slice(-2),
      );
      ...
      stringTime = stringTime.replace(
        /hh/,
        ('0' + value.created.getHours()).slice(-2),
      );
      stringTime = stringTime.replace(
        /mm/,
        ('0' + value.created.getMinutes()).slice(-2),
      );
      stringTime = stringTime.replace(
        /ss/,
        ('0' + value.created.getSeconds()).slice(-2),
      );
  ...
  async save() {
    ...
    document.getElementById('axp_saveload_span_message').textContent =
      'セーブするスロットを選択（※保存済みのスロットは上書きされます）';
    try {
      await this.dbSystem.loadEntry(
        'save',
        STORE_NAME_SAVE_MANUAL,
        this.createSlotHTML,
      );
      // スロット要素（直下の子要素div）を取得
      const elementsSlot = document.querySelectorAll(
        '#axp_saveload_div_insertHTML > div',
      );
          ...
          (async () => {
            try {
              ...
              if (
                ...
                alert(
                  `データの画像サイズ(横:${data.x_max} 縦:${data.y_max}) が許容値を超えています。\nこのデータをロードすることはできません。`,
                );
                return;
              }
  ...
  restore() {
    ...
    document.getElementById('axp_saveload_span_message').textContent =
      '自動バックアップ（10ストローク毎に最大20件まで保存）を選択（※現在の描画内容は破棄されます）';
  ...
  restore_oekaki_id(readdata) {
    ...
    if (
      readdata.draftImageFile !== undefined &&
      readdata.draftImageFile !== null
    ) {
      // 「draftImageFile」画像の場合
      console.log(
        '同一掲示板チェック:',
        readdata.oekaki_bbs_pageno,
        this.axpObj.post_bbs_pageno,
      );
      // 同一掲示板チェックが有効
      ...
      readdata.oekaki_id !== undefined &&
      readdata.oekaki_id !== null
    ) {
      // 「oekaki_id」画像の場合
      console.log(
        '同一掲示板チェック:',
    ...
    return new Promise((resolve, reject) => {
      ...
      openReq.onupgradeneeded = (event) => {
        ...
        if (event.oldVersion <= 1) {
          const store_manual = db.createObjectStore(STORE_NAME_SAVE_MANUAL, {
            keyPath: 'id',
          });
          ...
          const store_auto = db.createObjectStore(STORE_NAME_SAVE_AUTO, {
            autoIncrement: true,
          });
          store_auto.createIndex('created', 'created', { unique: false });

          const store_config = db.createObjectStore(STORE_NAME_CONFIG, {
            keyPath: 'id',
          });
          store_config.put({ id: 'config_01' });

          const store_palette = db.createObjectStore(STORE_NAME_PALETTE, {
            keyPath: 'id',
          });
