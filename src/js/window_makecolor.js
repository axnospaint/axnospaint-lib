...
export class ColorMakerSystem extends ToolWindow {
  ...
  getDefaultPosition() {
    return {
      ...
      left:
        this.axpObj.paintBodyElement.clientWidth - this.window_width - 120 - 10,
      top: 250,
    };
  }
  // 初期化
  ...
  startEvent() {
    // ボタン：スワップ
    document
      .getElementById('axp_makecolor_button_swapColor')
      .addEventListener('click', () => {
        ...
        this.axpObj.msg(
          '@COL0003',
          hex2rgb(this.maincolor),
          hex2rgb(this.subcolor),
        );
      });
    // ボタン：パレット登録
    document
      .getElementById('axp_makecolor_button_addColor')
      .addEventListener('click', () => {
    ...
    document.getElementById('axp_makecolor_range_red').oninput =
      oninputRangeColor;
    document.getElementById('axp_makecolor_range_green').oninput =
      oninputRangeColor;
    document.getElementById('axp_makecolor_range_blue').oninput =
      oninputRangeColor;
    ...
    const onchangeColorValue = () => {
      ...
      //  RGBの取得
      var r = adjustColorValue(
        document.getElementById('axp_makecolor_number_red').value,
      );
      var g = adjustColorValue(
        document.getElementById('axp_makecolor_number_green').value,
      );
      var b = adjustColorValue(
        document.getElementById('axp_makecolor_number_blue').value,
      );
    ...
    document.getElementById('axp_makecolor_number_red').onchange =
      onchangeColorValue;
    document.getElementById('axp_makecolor_number_green').onchange =
      onchangeColorValue;
    document.getElementById('axp_makecolor_number_blue').onchange =
      onchangeColorValue;
  ...
  setMainColor(colorcode, changer = null) {
    ...
    // ＠サブカラー
    if (
      document.getElementById('axp_makecolor_div_subColor').dataset.selected ===
      'true'
    ) {
      this.subcolor = colorcode;
      // サブカラー表示
      ...
      // カラーコード(HEX)表示
      document.getElementById('axp_makecolor_text_colorCode').value =
    ...
    document.getElementById('axp_makecolor_range_red').value = parseInt(
      rgbcolor[0],
      16,
    );
    document.getElementById('axp_makecolor_range_green').value = parseInt(
      rgbcolor[1],
      16,
    );
    document.getElementById('axp_makecolor_range_blue').value = parseInt(
      rgbcolor[2],
      16,
    );
    document.getElementById('axp_makecolor_number_red').value = parseInt(
      rgbcolor[0],
      16,
    );
    document.getElementById('axp_makecolor_number_green').value = parseInt(
      rgbcolor[1],
      16,
    );
    document.getElementById('axp_makecolor_number_blue').value = parseInt(
      rgbcolor[2],
      16,
    );
    ...
    let temporary = document.querySelector(
      '#axp_makecolor_div_mixedPalette>div>div[data-selected="true"]',
    );
    if (temporary) {
      // メインカラー用表示を混色にする（CSS変数経由で疑似要素に参照させる）
      document
        .getElementById('axp_makecolor_div_mainColor')
        .style.setProperty('--axp-mixedcolor', temporary.dataset.color);
    }
  }
  ...
  get addPaletteName() {
    ...
    if (temporary) {
      ...
      if (
        document.getElementById('axp_makecolor_div_subColor').dataset
          .selected === 'true'
      ) {
        return 'サブカラー';
      } else {
  ...
  displayColorMaker() {
    ...
    // カラーコード(HEX)表示
    document.getElementById('axp_makecolor_text_colorCode').value =
      this.maincolor.toUpperCase();
    // サブカラー表示
    document.getElementById(
      'axp_makecolor_div_subColor',
    ).style.backgroundColor = this.subcolor;
  ...
  selectPalette(target = 'none') {
    ...
    if (typeof target === 'object') {
      ...
      document.getElementById('axp_makecolor_div_mainColor').dataset.selected =
        'false';
      document.getElementById('axp_makecolor_div_subColor').dataset.selected =
        'false';
      document.getElementById(
      ...
      document.getElementById('axp_makecolor_div_mainColor').dataset.mixed =
        'true';
    } else {
      // 混色パレット以外
      document.getElementById('axp_makecolor_div_mainColor').dataset.mixed =
  ...
  createTemporaryPalette() {
    ...
    const createPaletteHTML = (colorcode, index) => {
      ...
      newDiv.addEventListener('pointerenter', (e) => {
        ...
        this.axpObj.msg(
          '@AXP2000',
          percentage,
          hex2rgb(e.target.dataset.color),
        );
      });
      // パレットがクリックされたとき
    ...
    for (let index = 0; index < 20; index++) {
      // 画面に作成したカラーパレットを追加
      box.appendChild(createPaletteHTML(this.adjust(index), index));
    }
  }
  // 混色パレット：更新
  updateTemporaryPalette() {
    const palettes = document.querySelectorAll(
      '#axp_makecolor_div_mixedPalette>div>div',
    );
    let index = 0;
