...
export class ColorPaletteSystem extends ToolWindow {
  ...
  init() {
    ...
    // パレット要素取得
    this.elementPaletteBox = document.getElementById(
  ...
  startEvent() {
    ...
    const edit = () => {
      ...
      if (this.isEditMode) {
        // ボタンを凹ませる
        document.getElementById('axp_palette_button_edit').dataset.selected =
          'true';
        // 削除ボタン表示
        UTIL.show('axp_palette_button_delete');
        // 列数スライダー表示
        if (
          document.getElementById('axp_config_checkbox_paletteColumnEdit')
            .checked
        ) {
        ...
        // ツールウィンドウの色を変化（注意喚起）
        document
          .getElementById('axp_palette')
          .classList.add('axpc_window_attention');
        ...
        document
          .getElementById('axp_palette')
          .classList.remove('axpc_window_attention');
        // パレット編集モードを解除しました。
    ...
    const p_delete = () => {
      ...
      for (idx = 0; idx < this.elementPaletteBox.childElementCount; idx++) {
        if (
          this.elementPaletteBox.children[idx] === this.elementCurrentPalette
        ) {
          break;
        }
      }
    ...
    // ボタン：登録
    document
      .getElementById('axp_palette_button_addColor')
  ...
  addcolor() {
    ...
    // 登録：パレットボックスの最後尾に追加
    this.elementCurrentPalette = box.appendChild(
      this.createPaletteHTML(colorcode),
    );
    // 新パレットを選択
    this.elementCurrentPalette.dataset.selected = 'true';
  ...
  drag_down(e) {
    ...
    ps_data.cloneName = palette_sorting.insertClone(
      target,
      palette_sorting.index(target),
    );
  ...
  createPalette() {
    ...
    // 最初の要素を選択済みにする
    this.elementCurrentPalette = document.querySelector(
      '#axp_palette_div_paletteBox :first-child',
    );
    this.elementCurrentPalette.dataset.selected = 'true';
    // 列数の更新
    this.updatePaletteColumn();
  ...
  updatePaletteColumn() {
    ...
    let elementPaletteBox = document.getElementById(
      'axp_palette_div_paletteBox',
    );
    ...
    // 設定側スライダー
    document.getElementById('axp_config_form_paletteColumnValue').volume.max =
      length + 1;
    document.getElementById('axp_config_form_paletteColumnValue').volume.value =
      range_index;
    document.getElementById('axp_config_form_paletteColumnValue').result.value =
      value;

    // 設定用カラーパレット表示更新
    this.axpObj.configSystem.dispPalettebox(
...
const palette_sorting = {
  ...
  swap(target) {
    ...
    const siblings = parent.querySelectorAll(
      `:scope > *:not(.axpc_onGRAB):not(.${ps_data.cloneName})`,
    );
    ...
    for (let thatIdx = 0, len = siblings.length; thatIdx < len; thatIdx++) {
      ...
      const hitX =
        targetRectX <= thatRectXHalf && targetRectX + targetW >= thatRectXHalf;
      const hitY =
