...
var ReinventedColorWheel = (function () {
      ...
      60 *
        (max === r
          ...
          : max === g
            ? (b - r) / delta + 2
            : (r - g) / delta + 4);
    return [h < 0 ? h + 360 : h, max && (delta * 100) / max, (max * 100) / 255];
  }
  ...
  function normalizeHsvOrDefault(hsv, defaultHsvOrHsl) {
    if (hsv) {
      return [
        ...
        isFiniteNumber(hsv[1])
          ? normalizePercentage(hsv[1])
          : defaultHsvOrHsl[1],
        isFiniteNumber(hsv[2])
          ? normalizePercentage(hsv[2])
          : defaultHsvOrHsl[2],
      ];
    } else {
        ...
        ? function (element, onDragStart, onDragMove) {
            ...
            element.addEventListener('touchstart', function (event) {
              if (
                ...
                onDragStart(event.touches[0]) !== false
              ) {
                dragging = true;
  ...
  var ReinventedColorWheel = /** @class */ (function () {
    function ReinventedColorWheel(options) {
      ...
      this.wheelDiameter =
        this.options.wheelDiameter || defaultOptions.wheelDiameter;
      this.wheelThickness =
        this.options.wheelThickness || defaultOptions.wheelThickness;
      this.handleDiameter =
        this.options.handleDiameter || defaultOptions.handleDiameter;
      this.onChange = this.options.onChange || defaultOptions.onChange;
      this.wheelReflectsSaturation =
        this.options.wheelReflectsSaturation !== undefined
          ? this.options.wheelReflectsSaturation
          : defaultOptions.wheelReflectsSaturation;
      this.rootElement = this.options.appendTo.appendChild(
        createElementWithClass('div', 'reinvented-color-wheel'),
      );
      this.hueWheelElement = this.rootElement.appendChild(
        createElementWithClass('canvas', 'reinvented-color-wheel--hue-wheel'),
      );
      this.hueWheelContext = this.hueWheelElement.getContext('2d');
      this.hueHandleElement = this.rootElement.appendChild(
        createElementWithClass('div', 'reinvented-color-wheel--hue-handle'),
      );
      this.svSpaceElement = this.rootElement.appendChild(
        createElementWithClass('canvas', 'reinvented-color-wheel--sv-space'),
      );
      this.svSpaceContext = this.svSpaceElement.getContext('2d');
      this.svHandleElement = this.rootElement.appendChild(
        createElementWithClass('div', 'reinvented-color-wheel--sv-handle'),
      );
      this._redrawHueWheel = function () {
        ...
        var hslPostfix = _this.wheelReflectsSaturation
          ? ','.concat(_this._hsl[1], '%,').concat(_this._hsl[2], '%)')
          : ',100%,50%)';
      ...
      this._hsv = normalizeHsvOrDefault(
        options.hsv
          ? options.hsv
          : options.hsl
            ? ReinventedColorWheel.hsl2hsv(options.hsl)
            : options.rgb
              ? ReinventedColorWheel.rgb2hsv(options.rgb)
              : options.hex
                ? ReinventedColorWheel.rgb2hsv(
                    ReinventedColorWheel.hex2rgb(options.hex),
                  )
                : undefined,
        defaultOptions.hsv,
      );
      ...
      var invertTransform = function (x, y) {
        var m = _this._inverseTransform.multiply(
          new Matrix('matrix(1,0,0,1,'.concat(x, ',').concat(y, ')')),
        );
        return { x: m.e, y: m.f };
      };
    ...
    Object.defineProperty(ReinventedColorWheel.prototype, 'hsl', {
      ...
      set: function (value) {
        tripletsAreEqual(this._hsl, value) ||
          this._setHSV(ReinventedColorWheel.hsl2hsv(value));
      },
      enumerable: false,
      configurable: true,
    });
    Object.defineProperty(ReinventedColorWheel.prototype, 'rgb', {
      ...
      set: function (value) {
        tripletsAreEqual(this._rgb, value) ||
          this._setHSV(ReinventedColorWheel.rgb2hsv(value));
      },
      enumerable: false,
      configurable: true,
    });
    ...
    /** @deprecated */ ReinventedColorWheel.prototype.setHSV = function () {
      this.hsv = arguments;
    };
    /** @deprecated */ ReinventedColorWheel.prototype.setHSL = function () {
      this.hsl = arguments;
    };
    ReinventedColorWheel.prototype.redraw = function () {
      ...
      this.svSpaceElement.width = this.svSpaceElement.height =
        (this.wheelDiameter - this.wheelThickness * 2) * Math.SQRT1_2 - 6;
      var hueHandleStyle = this.hueHandleElement.style;
      var svHandleStyle = this.svHandleElement.style;
      hueHandleStyle.width =
        hueHandleStyle.height =
        svHandleStyle.width =
        svHandleStyle.height =
          ''.concat(this.handleDiameter, 'px');
      hueHandleStyle.marginLeft =
        hueHandleStyle.marginTop =
        svHandleStyle.marginLeft =
        svHandleStyle.marginTop =
    ...
    ReinventedColorWheel.prototype._redrawHueHandle = function () {
      ...
      var hueHandleStyle = this.hueHandleElement.style;
      hueHandleStyle.left = ''.concat(
        wheelRadius * Math.cos(angle) + center,
        'px',
      );
      hueHandleStyle.top = ''.concat(
    ...
    ReinventedColorWheel.prototype._redrawSvHandle = function () {
      ...
      var hsv = this._hsv;
      svHandleStyle.left = ''.concat(
        offset + (svSpaceElementWidth * hsv[1]) / 100,
        'px',
      );
      svHandleStyle.top = ''.concat(
        offset + svSpaceElementWidth * (1 - hsv[2] / 100),
        'px',
      );
    };
