...
export class PenSystem extends ToolWindow {
  ...
  init() {
    ...
    document
      ...
      .insertAdjacentHTML(
        'beforeend',
        this.axpObj.translateHTML(html_pen_subwindow),
      );
    ...
    this.CANVAS.draw_ctx = this.CANVAS.draw.getContext('2d', {
    ...
    // ペンの太さプレビューキャンバス
    this.CANVAS.pensize = document.getElementById(
    ...
    // メイン
    this.penObj['axp_penmode_round'] = new Round({
      axpObj: this.axpObj,
      CANVAS: this.CANVAS,
    });
    this.penObj['axp_penmode_eraser_round'] = new Eraser({
      axpObj: this.axpObj,
      CANVAS: this.CANVAS,
    });
    this.penObj['axp_penmode_eraser_dot'] = new EraserDot({
      axpObj: this.axpObj,
      CANVAS: this.CANVAS,
    });

    this.penObj['axp_penmode_fill'] = new Fill({
      axpObj: this.axpObj,
      CANVAS: this.CANVAS,
    });
    this.penObj['axp_penmode_fillgradation'] = new Fillgradation({
      axpObj: this.axpObj,
      CANVAS: this.CANVAS,
    });

    this.penObj['axp_penmode_hand'] = new Hand({
      axpObj: this.axpObj,
      CANVAS: this.CANVAS,
    });
    this.penObj['axp_penmode_spuit'] = new Spuit({
      axpObj: this.axpObj,
      CANVAS: this.CANVAS,
    });
    // サブ
    this.penObj['axp_penmode_square'] = new Square({
      axpObj: this.axpObj,
      CANVAS: this.CANVAS,
    });
    this.penObj['axp_penmode_move'] = new Move({
      axpObj: this.axpObj,
      CANVAS: this.CANVAS,
    });
    this.penObj['axp_penmode_dot'] = new Dot({
      axpObj: this.axpObj,
      CANVAS: this.CANVAS,
    });
    this.penObj['axp_penmode_fude'] = new Fude({
      axpObj: this.axpObj,
      CANVAS: this.CANVAS,
    });
    this.penObj['axp_penmode_crayon'] = new Crayon({
      axpObj: this.axpObj,
      CANVAS: this.CANVAS,
    });
    this.penObj['axp_penmode_brush'] = new Brush({
      axpObj: this.axpObj,
      CANVAS: this.CANVAS,
    });

    // サブメニューをメインメニューに反映
    let elementsButton = document.querySelectorAll(
      '#axp_pen_div_rightSide>div>button',
    );
    let elementsButtonSub = document.querySelectorAll(
      '#axp_penmode>div>article>button:first-of-type',
    );
  ...
  resetCanvas() {
    ...
    this.CANVAS.brush.height =
      this.axpObj.y_size * this.axpObj.CONST.DRAW_MULTI;
  }
  // ペンツール変更（メインボタン）
  switchMainButton(element, caller = null) {
    ...
    if (
      target.dataset.idx !== '4' &&
      !caller &&
      target.dataset.selected === 'true'
    ) {
      let targetElements_button = document.querySelectorAll(
        '#axp_pen_div_rightSide>div>button',
      );
      ...
      let targetElements_article = document.querySelectorAll(
        '#axp_penmode>div>article',
      );
      ...
      // 種別名表示
      document.getElementById('axp_penmode_span_modeName').textContent =
        `${subwindow.dataset.name}${this.axpObj._('@PEN.TYPE')}`;
      ...
      // サブボタンメニューの表示座標指定
      let rect = document
        .getElementById('axp_pen_div_leftSide')
        .getBoundingClientRect();
  ...
  switchSubButton(element) {
    ...
    const elementButtonMain = document.querySelector(
      `#axp_pen_div_rightSide>div>button[data-idx="${target.dataset.idx}"]`,
    );
    // メインボタンにセットされているサブボタンclassを消去
    console.log(elementButtonMain);
    elementButtonMain.classList.remove(
      this.getClassIcon(elementButtonMain.dataset.set),
    );
  ...
  switchButton(elementButton) {
    // 選択状態の全クリア
    const elems_penicon = document.querySelectorAll(
      '#axp_pen_div_rightSide>div>button',
    );
  ...
  startEvent() {
    ...
    // ペンツールのサブウィンドウ
    document.getElementById('axp_penmode').addEventListener('click', (e) => {
      // モーダルウィンドウを閉じる
    ...
    // レンジスライダー：ペンの太さ
    document
      .getElementById('axp_pen_range_penSize')
    ...
    // レンジスライダー：ペンの不透明度
    document
      .getElementById('axp_pen_range_alpha')
      .addEventListener('input', (e) => {
    ...
    // レンジスライダー：塗り残し補正
    document
      .getElementById('axp_pen_range_fillThreshold')
      .addEventListener('input', (e) => {
        ...
        this.axpObj.configSystem.saveConfig(
          'P-THR_' + this.pen_mode,
          threshold,
        );
      });
    // レンジスライダー：角度
    document
      .getElementById('axp_pen_range_fillGradationDeg')
      .addEventListener('input', (e) => {
    ...
    // レンジスライダー：トーン濃度
    document
      .getElementById('axp_pen_range_toneLevel')
      .addEventListener('input', (e) => {
        ...
        // コンフィグオブジェクトをDBに保存
        this.axpObj.configSystem.saveConfig(
          'P-TON_' + this.pen_mode,
          tone_level,
        );
      });
    // レンジスライダー：ぼかし度
    document
      .getElementById('axp_pen_range_blur')
      .addEventListener('input', (e) => {
        ...
        // コンフィグオブジェクトをDBに保存
        this.axpObj.configSystem.saveConfig(
          'P-BLU_' + this.pen_mode,
          blur_level,
        );
      });
    // レンジスライダー：丸み
    document
      .getElementById('axp_pen_form_radius')
      .addEventListener('input', (e) => {
    ...
    const setPenSizeByPreview = (e) => {
      ...
      if (e.target.classList.contains('axpc_onGRAB')) {
        ...
        var size = Math.min(
          200,
          Math.max(1, parseInt((basesize * 100) / this.axpObj.scale)),
        );
    ...
    this.CANVAS.pensize.addEventListener(
      'pointerleave',
      setPenSizeByPreview_end,
    );
  }
  ...
  move(x, y, e) {
    ...
    if (this.axpObj.codeCHANGE_SIZE_KEY) {
      if (this.base_index !== null) {
        ...
        let index = Math.trunc(
          Math.max(1, Math.min(100, Number(this.base_index) + difference_x)),
        );
    ...
    // ペンカーソル表示
    if (
      e.target.id === this.axpObj.CANVAS.main.id ||
      e.target.id === this.axpObj.ELEMENT.view.id
    ) {
    ...
    if (
      this.penObj[exec_mode].type === 'draw' ||
      this.penObj[exec_mode].type === 'eraser'
    ) {
      const stabilizer_value = Number(
  ...
  getToneLevel() {
    return this.penObj[this.pen_mode].toneLevel !== undefined
      ? this.penObj[this.pen_mode].toneLevel
      : null;
  }
  ...
  getBlurLevel() {
    return this.penObj[this.pen_mode].blurLevel !== undefined
      ? this.penObj[this.pen_mode].blurLevel
      : null;
  }
  ...
  getRadius() {
    return this.penObj[this.pen_mode].radius !== undefined
      ? this.penObj[this.pen_mode].radius
      : null;
  }
  setRadius(radius) {
    // セット可能チェック
  ...
  resetPenStyle() {
    Object.keys(this.penObj).forEach((key) => {
      console.log('key', key);
  ...
  changePenModeTemporary(mode) {
    ...
    switch (mode) {
      case 'axp_penmode_spuit':
        ...
        document.getElementById('axp_pen_button_spuitBase').dataset.selected =
          'true';
        break;
  ...
  changePenMode(mode) {
    ...
    // ペンの太さ
    displaySlider('axp_pen_form_penSize', this.getSize());
    // 不透明度
    displaySlider('axp_pen_form_alpha', this.getAlpha());
    // バケツ閾値
    displaySlider('axp_pen_form_fillThreshold', this.getThreshold());
    // バケツ角度
    displaySlider('axp_pen_form_fillGradationDeg', this.getGradation());
    // クレヨンの丸み
    displaySlider('axp_pen_form_radius', this.getRadius());
    // トーン濃度
    displaySlider(
    ...
    displaySlider(
      ...
      document.getElementById('axp_config_form_stabilizerValue').volume.value,
      type === 'draw' &&
        document.getElementById('axp_config_checkbox_stabilize').checked,
    );
    ...
    if (this.penObj[this.pen_mode].usePenPreview) {
      document.getElementById('axp_pen_div_preview').style.visibility =
        'visible';
    } else {
      document.getElementById('axp_pen_div_preview').style.visibility =
        'hidden';
    ...
    // 選択されたペンの名称をウィンドウに表示
    document.getElementById('axp_pen_span_penName').textContent =
      this.getName();
  }
  previewPenSize() {
  ...
  spuit(e) {
    ...
    var x = parseInt(
      ((e.clientX - clientRect_draw.left) * 100) / this.axpObj.scale,
    );
    var y = parseInt(
      ((e.clientY - clientRect_draw.top) * 100) / this.axpObj.scale,
    );
    // 座標のドットを読み取る
  ...
  fillAll() {
    ...
    this.axpObj.undoSystem.setUndo({
      ...
      layerObj:
        this.axpObj.layerSystem.layerObj[
          this.axpObj.layerSystem.getLayerIndex(targetId)
        ],
    });
    ...
    if (
      ...
      this.CANVAS.brush_ctx.fillStyle = createTonePattern(
        toneLevel,
        adjustColor,
      );
    } else {
    ...
    // 全面塗り潰しデータを作成
    this.CANVAS.brush_ctx.clearRect(
      0,
      0,
      this.axpObj.x_size,
      this.axpObj.y_size,
    );
    this.CANVAS.brush_ctx.fillRect(
      0,
      0,
      this.axpObj.x_size,
      this.axpObj.y_size,
  ...
  changePenAlpha(type) {
    ...
    if (alpha === null) {
      ...
      // コンフィグオブジェクトをDBに保存
      this.axpObj.configSystem.saveConfig(
        'P-ALP_' + this.axpObj.penSystem.pen_mode,
        alpha,
      );
    }
  }
